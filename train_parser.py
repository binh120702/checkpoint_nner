...
def generate_parser():
    ...
    parser.add_argument("--version", type=str, default="genia",
                        choices=["ace05", "ace04", "genia91", "kbp", "sagri"],
...
def generate_config(args):
    ...
    if args.word:
        ...
        if args.version.find("sagri") >= 0:
            word_embedding_config['path'] = 'data/sagri/BioWordVec_PubMed_MIMICIII_d200.npy'
            word_embedding_config['dim'] = 200
            word_embedding_config['padding_idx'] = 51154        
    ...
    if args.char:
        ...
        if args.version.find("sagri") >= 0:
            char_embedding_config['padding_idx'] = 205
        if args.version.find("ace04") >= 0:
            char_embedding_config['padding_idx'] = 200
        if args.version.find("kbp") >= 0:
    ...
    if args.pos:
        ...
        if args.version.find("sagri") >= 0:
            pos_embedding_config['padding_idx'] = 20
        if args.version.find("kbp") >= 0:
            pos_embedding_config['padding_idx'] = 46
    else:
